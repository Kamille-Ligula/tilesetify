  ...
  for (let j=0; j<height/tileHeight; j++) {
    for (let i=0; i<width/tileWidth; i++) {
      ...
      for (let k=0; k<tilesImgs.length; k++) {
        ...
        for (let m=0; m<tileWidth; m++) {
          for (let n=0; n<tileHeight; n++) {
            if (map.getPixelColor(i*16+m, j*16+n) === map.getPixelColor(tilesX[k]*16+m, tilesY[k]*16+n)) {
              pixels = 'same';
            }
            else {
              pixels = 'different';
      ...
      if (pixels === 'different' || tilesImgs.length === 0) {
        const newTile = await new Jimp(16, 16, async (err, tile) => {
          for (let k=0; k<16; k++) {
            for (let l=0; l<16; l++) {
              tile.setPixelColor( map.getPixelColor(i*16+k, j*16+l), k, l );
            }
          }
        });
